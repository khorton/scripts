tell application "iPhoto"
	activate
	try
		set clip_in to "44.45579528,-87.97536472"
		...
		if num_items < 2 then
			...
			error "Input has less than 2 items." number 9004
			
		else if num_items is greater than 2 then
			--display dialog "The clipboard has too many items" buttons {"OK"} with icon caution with title "ERROR"
			error "Input has more than 2 items." number 9005
			--else
		...
		try
			--set dialogResult to display dialog "Paste the following data into photo? Latitude: " & lat & "Longitude: " & long with title "Confirm Paste" buttons {"Cancel", "Paste"} default button "Paste" with icon 1
